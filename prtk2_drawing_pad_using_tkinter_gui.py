...
def start_draw(event):
    # using the global variables locally in the function using global keyword
    global last_x, last_y
    # event.x and event.y is for mouse coordinates and every drag event gives us new coordinates
...
def draw(event):
    ...
    # gets the starting of the drag event
    global last_x, last_y


    # if there is coordinate value then the code executes
    if last_x and last_y:
        # Create line method draws the line from the old coordinates last_x and last_y
        # to current coordinates event.x and event.y. Also setting brush color and size
...
def pick_color():
    # Using the global variable brush_color locally using global keyword
    global brush_color
    # askcolor method opens a GUI color picker dialog window.
    # By default, it returns a tuple of two elements, first is RGB value and second is hex value
...
def save_canvas_image():
    ...
    # Tkinter’s winfo_rootx()/winfo_rooty() give the absolute top-left coordinates
    # of the canvas widget on the screen. Normally, we can add winfo_width() and
    # winfo_height() to get the bottom-right corner. However, depending on system
    # DPI scaling and how Tkinter renders, ImageGrab.grab() may capture slightly
    # off regions (extra space on one side, clipping on another). To fix this,
    # we add manual offsets:
    #   +15 pixels to x  → removes unwanted left padding
    #   +100 pixels to width/height → ensures the bottom and right edges are not cut off
...
# from_= and to= sets the range of the value
